...
export default function VisualizationPlayer({ content }: { content: Pick<PatternContent, 'patternId' | 'steps' | 'initialValues'> }) {
  ...
  useEffect(() => { setIndex(0); setPlaying(false) }, [content.patternId])
  useEffect(() => {
    if (!playing || index >= steps.length-1) return
    const timer = setTimeout(() => setIndex(i => i+1), 1600/speed)
  ...
  return <section className="visualization" aria-label="Algorithm walkthrough">
    ...
    <div className="playback-controls"><button onClick={()=>{setIndex(0);setPlaying(false)}}>Reset</button><button disabled={index===0} onClick={()=>{setPlaying(false);setIndex(i=>i-1)}}>Previous</button><button className="primary" onClick={()=>{if(index===steps.length-1)setIndex(0);setPlaying(p=>!p)}}>{playing && index<steps.length-1 ? 'Pause' : 'Play'}</button><button disabled={index===steps.length-1} onClick={()=>{setPlaying(false);setIndex(i=>i+1)}}>Next</button><label>Speed <select value={speed} onChange={e=>setSpeed(Number(e.target.value))}><option value={0.5}>0.5×</option><option value={1}>1×</option><option value={2}>2×</option></select></label></div>
